...
public class Viper implements Role {
	...
	public void run() {
		while(true){
			try {
				RobotInfo[] enemiesWithinRange = rc.senseHostileRobots(rc.getLocation(), RobotType.VIPER.attackRadiusSquared);
				...
				if (enemiesSeen.length > 0) {
					...
					if (rc.isWeaponReady()) {
						enemiesWithinRange = rc.senseHostileRobots(rc.getLocation(), RobotType.VIPER.attackRadiusSquared);
